...
class EstadoCuentaServicePort(Protocol):
    async def crear_cargo(
        ...
        alumno_id: int,
        fecha: date,
        categoria_pago_id: Optional[int],
        monto: Decimal,
        referencia: str,
...
class GenerarCargosInicialesCommand(BaseModel):
    alumno_id: int
    sede_id: int                     # 🔹 solo para job/ contexto
    fecha: date
...
class GenerarCargosInicialesUseCase:
    def __init__(
        self,
        estado_cuenta_service: EstadoCuentaServicePort,
        uow: UnitOfWork,
    ...
    async def execute(self, cmd: GenerarCargosInicialesCommand) -> int:
        cargo_id = await self.estado_cuenta_service.crear_cargo(
        ...
        # Recalcular recordatorios de pago para este alumno/sede
