...
from app.kernel.domain.finanzas.errors import ArqueoYaExiste, ArqueoPeriodoInvalido
...
class GenerarArqueoMensualUseCase:
    ...
    async def execute(self, cmd: GenerarArqueoMensualCommand) -> ArqueoCaja:
        if not (1 <= cmd.mes <= 12 and cmd.anio >= 2000):
            raise ArqueoPeriodoInvalido("Período inválido")  # valida período [attached_file:33]
        ...
        if existe:
            raise ArqueoYaExiste(cmd.sede_id, f"{cmd.anio}-{cmd.mes:02d}")  # no duplicar arqueo [attached_file:33]
